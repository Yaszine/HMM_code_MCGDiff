...
class EpsilonNetFromScore(nn.Module):
    ...
    def forward(self, x, t):
        ...
        if not self.bias:
            return coeff * score
        return coeff * (score + nn.randn_like(score)*self.bias_std + self.bias_mean)
...
class Expandednet(nn.Module):
    ...
    def forward(self, x, t):
        return self.base_net(x.flatten(1, len(x.shape)-1), t[0]).reshape(x.shape[0], *self.expanded_size, requires_grad=True)